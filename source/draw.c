...
{
    if (total > 0)
        DrawStringF(SCREEN_WIDTH - 40, SCREEN_HEIGHT - 20, FONT_COLOR, BG_COLOR, "%3llu%%", (current * 100) / total);
    else
...
void DrawSplash(const char* splash_file, u32 use_top_screen) {
    ...
    if (!FileOpen(path)) {
        ...
        if (!FileOpen(path)) {
            DrawStringF(50, 200, FONT_M_COLOR, BG_M_COLOR, "Not found: %s", splash_file);
            return;
        }
    }
    if (use_top_screen) { // Load the splash image - top screen
        FileRead(TOP_SCREEN0, 400 * 240 * 3, 0);
        memcpy(TOP_SCREEN1, TOP_SCREEN0, 400 * 240 * 3);
    } else { // Load the splash image - bottom screen
